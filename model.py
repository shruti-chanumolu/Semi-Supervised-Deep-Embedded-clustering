...
class VAE(nn.Module):
    def __init__(self, label, image_size, channel_num, kernel_num, z_size):
        ...
        self.decoder = nn.Sequential(
            ...
            self._deconv(kernel_num // 4, channel_num, activation=0),
            nn.Sigmoid()
        )

    def forward(self, x, vae=1):
        # encode x
        encoded = self.encoder(x)

        # sample latent code z from q given x.
        mean, logvar = self.q(encoded)
        z = self.z(mean, logvar, vae)
    ...
    def _deconv(self, channel_num, kernel_num, activation=1):
        if(activation):
            return nn.Sequential(
                nn.ConvTranspose2d(
